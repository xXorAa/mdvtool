...
int check_preamble(unsigned char *p, int zeros) {
  if (!isbyte(p, 0, zeros)) return -1;

  if (!isbyte(p + zeros, 0xff, 2)) return -1;
...
void file_dump_chain(int f) {
  ...
  // dump block chain
  for (j = 0; j < 256; j++) {
    if (files[f][j] != 255) printf("%s%d", j ? ", " : "", files[f][j]);
  }
  printf("\n");
}
    ...
    if (hdr->ff != 0xff) {
      ...
      if (sec->data_csum != sum(sec->data, 512)) {
        printf("WARNING: Sector @%d(%d): Data checksum failed %x != %x\n", i,
               hdr->snum, sec->data_csum, sum(sec->data, 512));
      }
...
int file_open(char *name) {
  ...
  int j;
  for (j = 1; j < entries; j++) {
    if (strcmp(file_name(j), name) == 0) return j;
  }
  return -1;
}
...
int file_exists(int i) { return (files[i][0] != 0xff); }
...
void show_file_entry(file_t *f) {
  printf("%16s %5d %s, V:%x, U:%d, B:%d", f->name, SWAP32(f->length),
         (f->type <= 1) ? (f->type ? "EXEC" : "DATA") : "????",
...
void mdv_show_sector_mapping() {
  ...
  for (i = 0; i < MAX_SECTORS; i++)
    if (sector_table[i] != 255) printf("%3d -> %3d\n", i, sector_table[i]);
}
...
void zip_import(char *name) {
  ...
  for (i = 0; i < entries; i++) {
    ...
    if (!file)
      ...
      const unsigned char *extra = zip_file_extra_field_get(
          zip, i, index, &x_id, &x_len, ZIP_FL_CENTRAL);
      if (x_id == 0xfb4a) {
...
void file_import(char *name) {
  ...
  uint32_t xtcc = 0;
  char xtccbuf[8];
  fseek(in, size - 8, SEEK_SET);
  fread(xtccbuf, 1, 8, in);
  if (strncmp(xtccbuf, "XTcc", 4) == 0) {
    xtcc = SWAP32(*(uint32_t *)(xtccbuf + 4));
  }
  if (xtcc) {
    printf("XTcc detected 0x%x\n", xtcc);
  }
...
void mdv_rename(char *name) {
  ...
  lname[10] = 0;
  while (strlen(lname) < 10) lname[strlen(lname)] = ' ';
